...
def test_run_backtest_api_returns_metrics_trades_and_events():
    ...
    response = client.post(
        ...
        json={
            ...
            "signal_bars": [
                ...
                {"date": "2026-01-02", "open": 10, "high": 10.4, "low": 9.9, "close": 10.2, "volume": 100, "amount": 1020},
                {"date": "2026-01-03", "open": 10, "high": 10.5, "low": 10.0, "close": 10.4, "volume": 100, "amount": 1040},
                {"date": "2026-01-04", "open": 10, "high": 11.0, "low": 10.4, "close": 10.9, "volume": 140, "amount": 1526},
                {"date": "2026-01-05", "open": 11, "high": 11.5, "low": 10.8, "close": 11.3, "volume": 150, "amount": 1695},
                {"date": "2026-01-06", "open": 11, "high": 11.5, "low": 10.8, "close": 11.3, "volume": 100, "amount": 1130},
                {"date": "2026-01-07", "open": 11, "high": 11.5, "low": 10.8, "close": 11.3, "volume": 100, "amount": 1130},
            ],
    ...
    assert data["signal_bars"][-1]["close"] == 11.3
    ...
    class FakeWestockClient:
        async def fetch_kline(self, symbol, period="day", limit=200, timeout=30):
            ...
            return [
                ...
                backtest_api.SignalBar(date=date(2026, 1, 2), open=10, high=10.4, low=9.9, close=10.2, volume=100, amount=1020),
                backtest_api.SignalBar(date=date(2026, 1, 3), open=10, high=10.5, low=10.0, close=10.4, volume=100, amount=1040),
                backtest_api.SignalBar(date=date(2026, 1, 4), open=10, high=11.0, low=10.4, close=10.9, volume=140, amount=1526),
                backtest_api.SignalBar(date=date(2026, 1, 5), open=11, high=11.5, low=10.8, close=11.3, volume=150, amount=1695),
                backtest_api.SignalBar(date=date(2026, 1, 6), open=11, high=11.5, low=10.8, close=11.3, volume=100, amount=1130),
                backtest_api.SignalBar(date=date(2026, 1, 7), open=11, high=11.5, low=10.8, close=11.3, volume=100, amount=1130),
            ]
...
def test_run_backtest_api_can_fetch_fund_nav_and_align_with_signal_bars(monkeypatch):
    class FakeFundNavClient:
        async def fetch_nav(self, fund_code, start_date=None, end_date=None, page_size=10000, timeout=15):
            ...
            return [
                backtest_api.FundNavPoint(date=date(2026, 1, 1), nav=1.0),
                backtest_api.FundNavPoint(date=date(2026, 1, 2), nav=1.0),
                backtest_api.FundNavPoint(date=date(2026, 1, 3), nav=1.0),
                backtest_api.FundNavPoint(date=date(2026, 1, 4), nav=1.0),
                backtest_api.FundNavPoint(date=date(2026, 1, 5), nav=1.0),
                backtest_api.FundNavPoint(date=date(2026, 1, 6), nav=1.0),
                backtest_api.FundNavPoint(date=date(2026, 1, 7), nav=1.0),
            ]
