...
from app.backtest.observation.judge import (
    DeterministicJudge,
    ObservationJudge,
    OBSERVATION_DAYS,
)
...
class BacktestEngine:
    ...
    async def run(
        ...
        *,
        ...
        batches = self._seed_initial_batches(
            config, shares, fund_nav[0].date, initial_nav
        )
        ...
        sell_trigger_map = {t.index: t for t in all_triggers if t.kind == "sell"}

        # ── Pass 2: pre-compute judgments for all triggers (cache + LLM) ─
        # This is the only async I/O phase. Results are cached so a warm
        # cache makes re-runs deterministic and zero-LLM.
        judge_version = getattr(judge, "PROMPT_VERSION", "deterministic-v1")
        judgments: dict[tuple[str, int], ObservationJudgment] = {}
        for tp in all_triggers:
            window_start = tp.index + 1
            window_bars = signal_bars[window_start : window_start + OBSERVATION_DAYS]
            j: ObservationJudgment | None = None
            cache_key: str | None = None
            if cache is not None:
                cache_key = compute_cache_key(
                    config.signal_code, tp, window_bars, judge_version
                )
                j = await cache.get(cache_key)
            if j is None:
                j = await judge.judge(tp, window_bars, config, all_bars=signal_bars)
        ...
        for index, nav_point in enumerate(fund_nav):
            ...
            if buy_observation_start_index <= index <= buy_observation_until_index:
                if (
                    index == buy_observation_until_index
                    and buy_observation_candidate is not None
                ):
                    # 观察期最后一天：读取 Pass 2 预计算的判定
                    trigger = buy_observation_candidate
                    judgment = judgments.get(
                        (trigger.kind, trigger.index),
                        ObservationJudgment(
                            decision="hold",
                            confirmed=False,
                            reason="判定缺失",
                    ...
                    if judgment.confirmed and judgment.decision == "buy":
                        ...
                        buy_observation_until_index = -1
                        buy_observation_candidate = None
            elif index in buy_trigger_map and snapshot.position_pct < min(
                config.target_position_pct, config.max_single_position_pct
            ):
                # 新的买入触发：进入观察期
                ...
                if test_mode:
                    ...
                    events.append(
                        self._event_record(
                            observe_event, decision, reporter, batch_skill_router
                        )
                    )
            ...
            if sell_observation_start_index <= index <= sell_observation_until_index:
                if (
                    ...
                    and sell_observation_candidate is not None
                ):
                    # 观察期最后一天：读取 Pass 2 预计算的判定
            ...
            if buy_hold_event:
                ...
                events.append(
                    self._event_record(
                        buy_hold_event, hold_decision, reporter, batch_skill_router
                    )
                )
            if sell_hold_event:
                hold_decision = StrategyDecision(
                ...
                events.append(
                    self._event_record(
                        sell_hold_event, hold_decision, reporter, batch_skill_router
                    )
                )
            ...
            for event in day_events:
                ...
                if is_core_protection and index <= core_sell_cooldown_until_index:
                    if test_mode:
                        ...
                        events.append(
                            self._event_record(
                                event, decision, reporter, batch_skill_router
                            )
                        )
                        emitted_event = True
                    continue
                ...
                events.append(
                    self._event_record(event, decision, reporter, batch_skill_router)
                )
                emitted_event = True
                ...
                if decision.action == "sell" and event.event_type in {
                    "profit_drawdown",
                    "technical_breakdown",
                }:
            ...
            if test_mode and not emitted_event:
                ...
                if triggers:
                    event = self._system_check_event(
                        signal_bars[index].date, snapshot, triggers
                    )
                    decision = StrategyDecision(
                        action="observe",
        ...
        for obs_candidate, obs_type in [
            ...
            hold_event = BacktestEvent(
                event_type=(
                    "buy_candidate" if obs_type == "buy" else "technical_breakdown"
                ),
            ...
            events.append(
                self._event_record(
                    hold_event, hold_decision, reporter, batch_skill_router
                )
            )
    ...
    def _audit_observe_reason(self, event: BacktestEvent) -> str:
        ...
        if "take_profit" in families:
            return (
                "出现止盈观察触发，但未命中任何基金批次自身止盈回撤条件，系统保持持有"
            )
    ...
    def _execute_pending(
        ...
        for event, decision in pending:
            if decision.action == "buy":
                ...
                if spend <= 0:
                    continue
                bought_shares, fee = self.fee_model.calculate_buy_shares(
                    spend, nav_point.nav
                )
                cash -= spend
                batch_type = self._next_buy_batch_type(
                    config, batches, nav_point.nav, equity
                )
                batches.append(
                    HoldingBatch(
                ...
                sell_batches = self._select_batches_to_sell(
                    event, decision, batches, nav_point.nav
                )
                if not sell_batches:
                    continue
                sold_any = False
                for batch in sell_batches:
                    ...
                    trades.append(
                        TradeRecord(
                            ...
                            fee=fee,
                            reason=self._batch_sell_reason(
                                event, decision, batch, batch_return_pct
                            ),
                            event_type=event.event_type,
    ...
    def _select_batches_to_sell(
        ...
        if event.event_type == "technical_breakdown":
            selected = [
                ...
                for batch in self._batches_by_priority(
                    batches, ["trial", "high_position", "confirmation"]
                )
        ...
        if event.event_type == "profit_drawdown":
            ...
            if isinstance(target_batch_type, str):
                selected = [
                    batch
                    for batch in self._batches_by_priority(batches, [target_batch_type])
            ...
            selected = [
                batch
                for batch in self._batches_by_priority(batches, ["confirmation"])
                if self._batch_take_profit_allowed(batch, nav)
            ]
        ...
        if event.event_type == "post_sell_observation":
            ...
            if source_event_type == "profit_drawdown":
                ...
                if event.details.get("core_sell_allowed"):
                    ...
                    observation_days_total = event.details.get(
                        "observation_days_total", 3
                    )
                    if observation_day >= observation_days_total:
                        # 最后一天：卖出全部剩余核心仓，彻底清仓
                        return [
                            batch for batch in batches if batch.batch_type == "core"
                        ]
                    else:
                        # 非最后一天：不卖（只观察）
                        return []
    ...
    def _batches_by_priority(
        self, batches: list[HoldingBatch], priority: list[str]
    ) -> list[HoldingBatch]:
    ...
    def _batches_covering_shares(
        ...
        for batch in self._batches_by_priority(
            batches, ["trial", "high_position", "confirmation", "core"]
        ):
            if remaining <= 1e-9:
                break
    ...
    def _update_batch_peaks(self, batches: list[HoldingBatch], nav: float) -> None:
        for batch in batches:
            batch.peak_return_pct = max(
                batch.peak_return_pct, self._batch_return_pct(batch, nav)
            )
    ...
    def _batch_breakdown_sell_allowed(self, batch: HoldingBatch, nav: float) -> bool:
        ...
        if batch.batch_type == "confirmation":
            return (
                current <= -10
                or (batch.peak_return_pct >= 8 and drawdown >= 5)
                or self._batch_cost_line_protection_triggered(batch, current)
            )
        return False
    ...
    def _batch_sell_reason(
        ...
        if (
            event.details.get("batch_protection") == "cost_line"
            and event.details.get("target_batch_type") == batch.batch_type
        ):
            return (
                f"{decision.reason}；{self._batch_label(batch.batch_type)}基金批次最高浮盈"
    ...
    def _detect_post_sell_observation(
        ...
        if (
            not source_event_type
            or index < start_index
            or index > until_index
            or not batches
        ):
            return []
        ...
        priority = (
            "P1" if source_event_type in {"technical_breakdown", "stop_loss"} else "P2"
        )
        family = (
            "stop_loss"
            if source_event_type in {"technical_breakdown", "stop_loss"}
    ...
    def _post_sell_recheck_result(
        ...
        if source_event_type in {"technical_breakdown", "stop_loss"}:
            ...
            if fragile_batches:
                labels = "、".join(
                    self._batch_label(batch.batch_type) for batch in fragile_batches
                )
                return True, f"{labels}仍命中批次止损/破位条件，继续交给卖出执行", False
            if self._signal_still_broken(index, bars, config) and any(
                batch.batch_type == "core" for batch in batches
            ):
                return (
                    True,
                    "参考标的仍在关键EXPMA下方，脆弱批次已处理，继续处理半个核心仓",
                    True,
                )
        ...
        if source_event_type == "profit_drawdown":
            ...
            if profit_batches:
                labels = "、".join(
                    self._batch_label(batch.batch_type) for batch in profit_batches
                )
            ...
            core_batches = [
                batch
                for batch in self._batches_by_priority(batches, ["core"])
                if self._core_protection_tier(
                    batch, self._batch_return_pct(batch, nav), market_regime
                )
                is not None
            ]
            if (
                core_batches
                and is_last_observation_day
                and self._signal_still_broken(index, bars, config)
            ):
                return (
                    True,
            ...
            return (
                False,
                "脆弱批次已处理，核心仓趋势已修复或无保护档位，停止卖出",
                False,
            )
        return False, "上一轮卖出来源不明确，卖后观察只记录不继续执行", False

    def _signal_still_broken(
    ...
    def _get_market_regime(
        ...
        if len(closes) < 60:
            ...
            return (
                "bull"
                if is_macro_bullish
                else ("neutral" if prosperity_score >= 5.0 else "bear")
            )

        # 计算 MACD（简化版：快线用 12 周期，慢线用 26 周期）
        try:
            macd_values = (
                expma(closes, 12) if len(closes) >= 12 else [closes[-1]] * len(closes)
            )
            signal_line = (
                expma(closes, 26) if len(closes) >= 26 else [closes[-1]] * len(closes)
            )
            macd = (
                macd_values[-1] - signal_line[-1]
                if len(macd_values) > 0 and len(signal_line) > 0
                else 0
            )
    ...
    def _merge_batch_events(
        ...
        return (
            detected_events[:first_sell_index]
            + batch_events
            + detected_events[first_sell_index:]
        )

    def _batch_values_by_type(
        self, batches: list[HoldingBatch], nav: float
    ) -> dict[str, float]:
        values: dict[str, float] = {}
        for batch in batches:
            values[batch.batch_type] = (
                values.get(batch.batch_type, 0.0) + batch.shares * nav
            )
        return values
    ...
    def _calculate_metrics(
        ...
        annual_return_pct = (
            pow(final_equity / config.initial_cash, 365 / days) - 1
        ) * 100
