...
- [ ] **Step 1: 写失败测试（FakeLLMClient 固定输出）** - confirmed=true/false/hold/解析失败四路径
- [ ] **Step 2: 跑确认失败**
- [ ] **Step 3: 实现 prompts.py + llm_judge.py** - temperature=0，`response_format={"type":"json_object"}`
- [ ] **Step 4: 测试通过**
- [ ] **Step 5: 提交** `feat: add LLMObserverJudge with structured skill-bound prompts`
...
- [ ] **Step 1: 写失败测试** - put 后 get 命中；skill 版本变后不命中；未 put 返回 None
- [ ] **Step 2-5: 实现 + 测试 + 提交** `feat: add persistent ObservationCache keyed by skill versions`
...
- [ ] **Step 1: 写失败测试** - fake judge + real cache：首次跑调 judge、写 cache；再次跑命中 cache 零调用，结果一致
- [ ] **Step 2: 跑确认失败**（同步 run 无法 await）
- [ ] **Step 3: 改 async** - run/execute_pending/相关方法 async；API await；测试 async
- [ ] **Step 4: 接 judge+cache**
- [ ] **Step 5: 全 backtest 测试绿**
- [ ] **Step 6: 提交** `feat: make engine async and use LLMObserverJudge with cache`
...
- [ ] **Step 1: 写失败测试** - fake skill 返回 bull/tight -> SectorRegime 正确；skill 不可用 -> 趋势兜底
- [ ] **Step 2-5: 实现 + 测试 + 提交** `feat: add SectorRegimeProvider using market-regime skill`
- [ ] **核对**：读 `~/.codex/skills/batch-trading-market-regime/SKILL.md`，确认其 inputs 能吃 signal-ETF 赛道数据；若不能，扩展 skill 的 references 或在 provider 里把赛道趋势拼进 prompt
...
- [ ] **Step 1: 写失败测试** - 跌破观察进行中时，止盈信号被压制（不卖）；趋势未破时止盈正常触发
- [ ] **Step 2-5: 实现 + 测试 + 提交** `feat: two-layer sell with risk-exit priority over profit-taking`
...
- [ ] **Step 2-5: 实现 + 测试 + 提交** `feat: regime-conditional profit-taking (bull trailing, bear gain-ladder)`
...
- [ ] **Step 1: 写测试** - config 改阈值 -> 引擎行为跟着变（证明不再硬编码）
- [ ] **Step 2: 外置** - 移到 config 字段 + presets 设值；引擎/profit_taking 读 config
- [ ] **Step 3: 测试 + 提交** `refactor: externalize backtest thresholds to config/presets`
...
- [ ] **Step 1: 写测试** - bear regime + 买确认 -> buy_scale 降档（买更少）；bull -> 全仓
- [ ] **Step 2: 去 hardcode，接 regime provider**
- [ ] **Step 3: 测试 + 提交** `feat: wire sector regime into buy sizing (remove hardcoded neutral)`
...
- [ ] **Step 1: 写测试** - fake skill 返回 1.5 -> TriggerScanner 用 1.5；不可用 -> config 默认
- [ ] **Step 2-5: 实现 + 测试 + 提交** `feat: skill-provided volume thresholds for trigger scanner`
...
- [ ] **Step 1: 改实现** - `StrategyCompilerAgent` 用 `get_llm_client` 的 `chat()`
- [ ] **Step 2: 现有 compiler 测试改 fake client 适配 registry 接口**
- [ ] **Step 3: 测试 + 提交** `refactor: route strategy compiler through LLM registry`
...
- [ ] **Step 1: 全 backtest 测试绿**（async 适配后）
- [ ] **Step 2: 契约测试**
  - LLM judge 缓存命中 -> 重跑结果一致（可复现）
  - 牛市 trailing 止盈、熊市 gain-ladder 止盈、风险出口优先
  - 买侧 bear 降档
  - regime 随时间变（同回测内牛转熊）
- [ ] **Step 3: 真实数据 smoke**（可选，需 LLM 凭证）- 跑一个 ETF 预设，确认卖侧 gain-ladder 真触发、churn 比Phase 0 降
- [ ] **Step 4: 提交** `test: phase1 regression and contract tests`
